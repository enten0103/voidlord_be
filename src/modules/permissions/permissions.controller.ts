...
export class PermissionsController {
    ...
    @ApiResponse({ status: 201, description: 'Granted', schema: { example: { userId: 12, permission: 'BOOK_UPDATE', level: 1 } } })
    @ApiResponse({ status: 400, description: 'Bad Request', schema: { example: { statusCode: 400, message: 'Invalid level', error: 'Bad Request' } } })
    ...
    @ApiResponse({ status: 201, description: 'Revoked', schema: { example: { revoked: true } } })
    @ApiResponse({ status: 400, description: 'Bad Request', schema: { example: { statusCode: 400, message: 'Target user not found', error: 'Bad Request' } } })
    @ApiResponse({ status: 401, description: 'Unauthorized', schema: { example: { statusCode: 401, message: 'Unauthorized', error: 'Unauthorized' } } })
    @ApiResponse({ status: 403, description: 'Forbidden (insufficient permission)', schema: { example: { statusCode: 403, message: 'Forbidden', error: 'Forbidden' } } })
