...
export class PermissionsService {
  ...
  async grant(currentUserId: number, dto: GrantPermissionDto) {
    if (![1, 2, 3].includes(dto.level))
      throw new BadRequestException('Invalid level');
    const currentUserPerm = await this.getUserPermissionLevel(
      currentUserId,
      dto.permission,
    );
  ...
  async revoke(currentUserId: number, dto: RevokePermissionDto) {
    ...
    if (!perm) throw new BadRequestException('Permission not found');
    const up = await this.userPermRepo().findOne({
      where: { user: { id: user.id }, permission: { id: perm.id } },
      relations: ['grantedBy'],
    });
    ...
    if (currentUserPerm === 2) {
      if (!up.grantedBy || up.grantedBy.id !== currentUserId) {
        throw new ForbiddenException(
          'Level 2 can only revoke assignments granted by themselves',
        );
      }
    }
  ...
  async getUserPermissionLevel(
    ...
    const res = await this.userPermRepo()
  ...
  async listUserPermissions(userId: number) {
    const ups = await this.userPermRepo().find({
