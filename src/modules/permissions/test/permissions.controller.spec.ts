...
describe('PermissionsController', () => {
  ...
  it('should grant permission', async () => {
    ...
    const res = await controller.grant(req, {
      userId: 1,
      permission: 'USER_READ',
      level: 1,
    } as any);
    expect(res).toEqual({ userId: 1, permission: 'USER_READ', level: 1 });
    expect(mockPermissionsService.grant).toHaveBeenCalledWith(10, {
      userId: 1,
      permission: 'USER_READ',
      level: 1,
    });
  ...
  it('should revoke permission', async () => {
    ...
    const req = { user: { userId: 10 } } as any;
    const res = await controller.revoke(req, {
      userId: 1,
      permission: 'USER_READ',
    } as any);
    expect(res).toEqual({ revoked: true });
    expect(mockPermissionsService.revoke).toHaveBeenCalledWith(10, {
      userId: 1,
      permission: 'USER_READ',
    });
  });
