...
export class PermissionGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        const meta = this.reflector.getAllAndOverride<{ permission: string; minLevel: number }>(PERMISSION_KEY, [
            context.getHandler(),
            context.getClass(),
        ]);
        ...
        if (!user?.userId) throw new ForbiddenException('Unauthenticated');

        // 超级管理员（基于用户名匹配）直接放行，减少每次查询数据库开销
        const superAdminUsername = process.env.ADMIN_USERNAME || 'admin';
        if (user.username === superAdminUsername) {
            Logger.debug(`Super admin bypass for ${user.username}`, 'PermissionGuard');
            return true;
        }
        ...
        const qb = this.dataSource.getRepository(UserPermission).createQueryBuilder('up')
            ...
            .innerJoin('up.user', 'u')
            .where('u.id = :uid AND p.name = :perm', { uid: user.userId, perm: permission })
            .select(['up.level AS level']);
        const raw = await qb.getRawOne<{ level: number }>();
        const level = raw?.level || 0;
        if (level < minLevel) {
            Logger.warn(`Permission denied: user ${user.userId} level ${level} < required ${minLevel} for ${permission}`, 'PermissionGuard');
            throw new ForbiddenException(`Need permission ${permission} (level >= ${minLevel})`);
        }
        return true;
    }
}
