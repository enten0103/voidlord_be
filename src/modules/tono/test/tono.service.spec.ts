...
describe('TonoService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: getRepositoryToken(ReaderEngine),
          useValue: mockEngineRepository,
        },
        {
          provide: getRepositoryToken(ReaderInstance),
  ...
  it('throws BadRequestException when book has no epub', async () => {
    ...
    await expect(service.parseBookToTono(1)).rejects.toThrow(
      BadRequestException,
    );
  });
  ...
  it('parses test.epub into tono.json and widget outputs', async () => {
    ...
    mockEngineRepository.findOne.mockResolvedValue({
      id: 1,
      key: 'tono',
    } as ReaderEngine);
    mockInstanceRepository.findOne.mockResolvedValue(null);
    mockInstanceRepository.create.mockImplementation(
      (v) => ({ ...v }) as ReaderInstance,
    );
  ...
  it('getWidget throws when widget json is missing', async () => {
    await expect(
      service.getWidget('book-1', 'OPS/missing.xhtml'),
