...
export class TonoService {
  ...
  async parseBookToTono(
    ...
    const { title, coverUrl, xhtmls, navItems, manifestItems } =
    ...
    for (const xhtmlPath of xhtmls) {
      ...
      await this.putJson(`${tonoPrefix}widgets/${xhtmlPath}.json`, widget);
    }
  ...
  async getAsset(
    hash: string,
    id: string,
  ): Promise<{ body: Readable; type: string; length?: number }> {
    const safeHash = this.normalizeSimpleHash(hash);
    const safeId = this.normalizePath(id);
  ...
  private async collectCssFromPath(
    ...
    visited.add(cssPath);
    const cssText = await this.getEpubObjectText(bookId, cssPath).catch(
      () => null,
    );
    if (!cssText) return;
  ...
  private buildPropertiesFromDeclarations(
    declarations: any[],
    ...
    for (const decl of declarations) {
      ...
      if (property === 'margin') {
        ...
        Object.assign(
          props,
          this.borderDirectionSegmentation(value, ['right']),
        );
        ...
        Object.assign(
          props,
          this.borderDirectionSegmentation(value, ['bottom']),
        );
    ...
    selector
      ...
      .forEach((s) => {
        ...
        const parts = rawParts.map((p) =>
          this.restoreAttributes(p, attributes),
        );
  ...
  private selectMatch(element: HTMLElement, selector: SelectorPart): boolean {
    ...
    const tag = (element.tagName || '').toLowerCase();
    if (selector.element && tag !== selector.element.toLowerCase())
      return false;
  ...
  private async inlineSvgImages(
    ...
    for (const img of images) {
      ...
      const data = await this.getEpubObjectBuffer(bookId, fullPath).catch(
        () => null,
      );
      if (!data) continue;
  ...
  private async getEpubObjectText(
    bookId: number,
    relativePath: string,
  ): Promise<string> {
  ...
  private async getEpubObjectBuffer(
    bookId: number,
    relativePath: string,
  ): Promise<Buffer> {
    const key = `books/${bookId}/epub/${relativePath}`;
