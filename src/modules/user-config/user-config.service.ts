...
export class UserConfigService {
    ...
    async getOrCreateByUserId(userId: number): Promise<UserConfig> {
        let cfg = await this.repo.findOne({ where: { user: { id: userId } }, relations: ['user'] });
        if (!cfg) {
            const user = await this.users.findOne({ where: { id: userId } });
            if (!user) throw new NotFoundException('User not found');
            cfg = this.repo.create({ user, locale: 'en', timezone: 'UTC', theme: 'light', email_notifications: true });
    ...
    async getPublicByUserId(userId: number) {
        const cfg = await this.repo.findOne({ where: { user: { id: userId } }, relations: ['user'] });
        if (!cfg) return { userId, avatar_url: null, display_name: null, bio: null };
        return {
            userId,
            avatar_url: cfg.avatar_url ?? (cfg.avatar_key ? this.files.getPublicUrl(cfg.avatar_key) : null),
            display_name: cfg.display_name ?? null,
            bio: cfg.bio ?? null,
        };
    }

    async updateMy(userId: number, dto: UpdateUserConfigDto): Promise<UserConfig> {
