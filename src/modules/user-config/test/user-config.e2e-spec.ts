...
describe('UserConfig (e2e)', () => {
    ...
    beforeAll(async () => {
        ...
        const testUser = { username: 'uc_user', email: 'uc@example.com', password: 'pass123456' };
        const reg = await request(app.getHttpServer()).post('/auth/register').send(testUser).expect(201);
        userId = reg.body.user.id;
        const login = await request(app.getHttpServer()).post('/auth/login').send({ username: testUser.username, password: testUser.password }).expect(201);
    ...
    it('/user-config/me (GET) should create default config on first access', async () => {
        const res = await request(app.getHttpServer()).get('/user-config/me').set('Authorization', `Bearer ${authToken}`).expect(200);
        expect(res.body.id).toBeDefined();
        expect(res.body.user.id).toBe(userId);
    });

    it('/user-config/me (PATCH) should update my config', async () => {
        const payload = { display_name: 'Tester', theme: 'dark', email_notifications: false };
    ...
    it('/user-config/:userId/public (GET) should return public profile', async () => {
        const res = await request(app.getHttpServer()).get(`/user-config/${userId}/public`).expect(200);
