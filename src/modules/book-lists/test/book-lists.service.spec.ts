...
describe('BookListsService', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: getRepositoryToken(FavoriteListItem), useValue: mockItemRepo },
  ...
  describe('create', () => {
    it('creates list', async () => {
      ...
      (listRepo.create as any).mockReturnValueOnce({ id: 1, name: 'Fav', is_public: false });
      (listRepo.save as any).mockResolvedValueOnce({ id: 1, name: 'Fav', is_public: false, created_at: new Date() });
    ...
    it('duplicate name', async () => {
      ...
      await expect(service.create(5, { name: 'Fav' })).rejects.toThrow(ConflictException);
    });
  });

  describe('listMine', () => {
    it('lists mine with counts', async () => {
      (listRepo.find as any).mockResolvedValueOnce([{ id: 1, name: 'A', is_public: false, created_at: new Date(), updated_at: new Date() }]);
  ...
  describe('getOne', () => {
    it('owner can view private', async () => {
      (listRepo.findOne as any).mockResolvedValueOnce({ id: 1, name: 'A', is_public: false, owner: { id: 5 }, created_at: new Date(), updated_at: new Date() });
      (itemRepo.find as any).mockResolvedValueOnce([]);
      const res = await service.getOne(1, 5);
      expect(res.id).toBe(1);
    });
    it('non-owner blocked private', async () => {
      (listRepo.findOne as any).mockResolvedValueOnce({ id: 1, is_public: false, owner: { id: 5 } });
      await expect(service.getOne(1, 6)).rejects.toThrow(ForbiddenException);
    });
    it('public visible', async () => {
      (listRepo.findOne as any).mockResolvedValueOnce({ id: 1, is_public: true, owner: { id: 5 }, created_at: new Date(), updated_at: new Date() });
      (itemRepo.find as any).mockResolvedValueOnce([]);
  ...
  describe('update', () => {
    it('updates name and description', async () => {
      (listRepo.findOne as any).mockResolvedValueOnce({ id: 1, name: 'Old', description: null, is_public: false, owner: { id: 5 } });
      (listRepo.findOne as any).mockResolvedValueOnce(null); // duplicate check
      (listRepo.save as any).mockResolvedValueOnce({ id: 1, name: 'New', description: 'Desc', is_public: true, updated_at: new Date() });
      const res = await service.update(1, 5, { name: 'New', description: 'Desc', is_public: true });
      expect(res.name).toBe('New');
      expect(res.is_public).toBe(true);
    });
    it('rejects not owner', async () => {
      (listRepo.findOne as any).mockResolvedValueOnce({ id: 1, owner: { id: 5 } });
      await expect(service.update(1, 6, { name: 'x' })).rejects.toThrow(ForbiddenException);
    });
    it('duplicate name change', async () => {
      (listRepo.findOne as any).mockResolvedValueOnce({ id: 1, name: 'Old', owner: { id: 5 } });
      (listRepo.findOne as any).mockResolvedValueOnce({ id: 2, name: 'New', owner: { id: 5 } });
      await expect(service.update(1, 5, { name: 'New' })).rejects.toThrow(ConflictException);
    });
  });
  ...
  describe('addBook/removeBook', () => {
    it('adds book', async () => {
      ...
      (itemRepo.save as any).mockResolvedValueOnce({ id: 11, added_at: new Date() });
    ...
    it('remove missing book', async () => {
      (listRepo.findOne as any).mockResolvedValueOnce({ id: 1, owner: { id: 5 } });
      (itemRepo.findOne as any).mockResolvedValueOnce(null);
      await expect(service.removeBook(1, 5, 9)).rejects.toThrow(NotFoundException);
    });
  });
});
