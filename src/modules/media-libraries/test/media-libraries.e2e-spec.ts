...
describe('MediaLibraries (e2e)', () => {
  ...
  beforeEach(async () => {
    ...
    const uReg = await request(httpServer)
      ...
      .send({
        username: 'ml_u',
        email: 'ml_u@example.com',
        password: 'pass1234',
      })
      .expect(201);
    const uData = parseBody(uReg.body, isAuthRegisterLite);
    userToken = uData.access_token;
    userId = uData.user.id;
    const oReg = await request(httpServer)
      .post('/auth/register')
      .send({
        username: 'ml_o',
        email: 'ml_o@example.com',
        password: 'pass1234',
      })
      .expect(201);
    const oData = parseBody(oReg.body, isAuthRegisterLite);
    otherToken = oData.access_token;
  ...
  it('library detail pagination returns subset with metadata', async () => {
    ...
    for (let i = 0; i < 3; i++) {
      const bRes = await request(httpServer)
        ...
        .expect(201);
      const bookObj = parseBody(bRes.body, (d): d is { id: number } => {
  ...
  it('virtual uploaded library supports pagination', async () => {
    ...
    const myBooksRes = await request(httpServer)
      ...
      .set('Authorization', `Bearer ${userToken}`)
      .expect(200);
    const totalBooks = Array.isArray(myBooksRes.body)
      ? myBooksRes.body.length
      : 0;
    // 分页获取虚拟库
