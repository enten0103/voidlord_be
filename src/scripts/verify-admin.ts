...
(async () => {
    ...
    console.log(`[verify-admin] Admin 用户存在: id=${admin.id}, username=${admin.username}, email=${admin.email}`);
    ...
    if (missingInDB.length) {
        console.warn(`[verify-admin] 数据库缺少权限记录: ${missingInDB.join(', ')}`);
    } else {
        console.log(`[verify-admin] 数据库权限总数=${allPerms.length} 与 PERMISSIONS 常量一致`);
    }
    ...
    if (missingAssignments.length === 0) {
        ...
        console.warn('[verify-admin] 缺失的权限分配: ' + missingAssignments.join(', '));
    }
    ...
    console.log(`[verify-admin] 统计: permissions_in_db=${allPerms.length}, assignments=${ups.length}`);
