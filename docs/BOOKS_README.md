...
> [!IMPORTANT]
> 本模块原先与“书籍收藏列表 (FavoriteList/FavoriteListItem)”旧体系存在耦合。该旧收藏列表功能已被完整移除，并由统一的 `MediaLibraries` 媒体库体系取代。请使用媒体库管理书籍分类/分组/进阶结构。旧收藏列表相关文档与代码均已删除或标记废弃，不再维护。

> [!NOTE]
> 若需要为图书建立「阅读进度 / 阅读记录」等结构，请使用用户自动生成的系统媒体库 `系统阅读记录`（在用户注册时创建，`is_system = true`，不可删除），并通过后续扩展 MediaLibraryItem 或附加统计表实现。阅读记录旧模块已废弃。
